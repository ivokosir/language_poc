...
import qualified TypeChecker                   as Type
...
getLines start end source =
  let
    indexedLines = zip (lines source) [1 ..]
    ...
    f (line, num) = if isInContext num
      then Just $ if isErrorLine num
...
formatTypeError :: String -> Type.Error -> String
formatTypeError source (Type.Error start end error) =
  getLines start end source ++ "\n" ++ error


printTypeErrors :: String -> [Type.Error] -> IO ()
printTypeErrors source errors =
  putStrLn $ intercalate "\n" $ fmap (formatTypeError source) errors
